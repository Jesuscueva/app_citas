...
class TraerCitasDeUsuarioController(generics.ListAPIView):
    queryset= UsuarioModel.objects.all()
    serializer_class= CitasPorUsuarioSerializaer
    def get_queryset(self, usuario):
        return UsuarioModel.objects.filter(usuarioNombre = usuario).first()
...
class CitaController(generics.RetrieveUpdateDestroyAPIView):
    ...
    def get_queryset(self, id):
        return CitaModel.objects.filter(citaId=id).find()
